...
# Deliberately NOT `set -e`: this hook's only safe failure mode is to
# block. If anything here errors out under `set -e`, the exit code is
# whatever bash chose, and whether that blocks or allows is undefined.
# Every path below ends in an explicit exit 0 (allow) or exit 2 (block).
set -uo pipefail
