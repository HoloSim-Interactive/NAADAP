...
| USAspending bulk | `https://files.usaspending.gov/award_data_archive/` — **split by top-tier agency, corrected 2026-09-15 from a live pull**: `FY{yyyy}_{agency}_Contracts_Full_{yyyymmdd}.zip`, DoD = `097` (Navy inside it as `awarding_sub_agency_code` 1700). FY2025 DoD: 1.04 GB compressed, ~8.6 GB across five CSVs, 297 columns. Bucket listing truncates at 1,000 keys — use `?prefix=FY2025_097`. Full PostgreSQL dump over 1.5 TB | FPDS-derived *transaction* rows (dedupe on `award_id_piid` for orders) including `parent_award_id_piid`, `awarding_office_code`, PSC, NAICS, `type_of_set_aside_code`, `idv_type_code`, `award_type_code`, `action_type_code`, `extent_competed_code`, business-size determination, and two description fields — `transaction_description` and `prime_award_base_transaction_description` (there is no `award_description` column) | no key |
