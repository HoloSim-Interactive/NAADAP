...
# Sonnet trial (client, 2026-09-02): this role's merge/label turns are
# mechanical and CI-gated; Sonnet draws ~2.5x less of the usage window.
# Revert to inherit if the bounce rate moves.
model: sonnet
memory: project
---
...
Scoped to this role and test-engineer only -- their comments are the pipeline's
...
- Exception: a blocked or conflicted merge keeps every detail the
  resolution needs (conflicting SHAs, the exact conflict). Brevity never trims evidence.
